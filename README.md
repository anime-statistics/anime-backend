...
Ручки `/ai/*` работают через Anthropic API. Ключ берётся из `Ai:ApiKey` или из
переменной окружения `ANTHROPIC_API_KEY`:

```bash
dotnet user-secrets set Ai:ApiKey "sk-ant-..." --project src/AnimeBackend.Api
```
...
Что делает каждая ручка: чат и стрим (обычный текст чанками, не SSE),
перефраз заметки в четырёх стилях, рекомендации и обработка голосового ввода.
Рекомендации получают выжимку коллекции как контекст, отвечают по JSON-схеме, а
каждый предложенный тайтл резолвится поиском по источникам в настоящий
`media_id` — выдуманные моделью названия отбрасываются. `deep_think` из
интерфейса переключает `effort`; `temperature` игнорируется, потому что текущие
модели Claude её не принимают.
