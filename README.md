...
Официальный адрес источника — `shikimori.io`, он и стоит в `appsettings.json`
по умолчанию. Старый `shikimori.one` заблокирован РКН и вдобавок рвёт
TLS-хендшейк с части сетей, использовать его нельзя. Если запросы к источнику
начнут висеть — проверьте `curl -v <BaseUrl>` и смените
`Sources:Shikimori:BaseUrl`: домены Шикимори мигрируют. Поиск при недоступном
источнике деградирует на живые.
